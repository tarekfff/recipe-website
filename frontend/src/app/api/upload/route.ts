...
export async function POST(request: Request) {
    try {
        ...
        let filename: string
        if (customName) {
            // Strip any extension they might have accidentally added to customName
            const baseName = customName.replace(/\.[^/.]+$/, '')
            const ext = path.extname(file.name)
            // Clean the filename so it's URL-friendly
            filename = `${baseName.replaceAll(/[^a-zA-Z0-9-_]/g, '-')}${ext}`
        } else {
